...
public class SchedulerApplication {
    ...
    public void run() throws ParseException {
        ...
        try {
            List<WebtoonDayList> webtoonDayList = this.webtoonDayListService.findBywebtoonDay(days.get(0));
            if (webtoonDayList == null) {
                webtoonData();
            }
            bookData();
            movieData();
    ...
    public void movieDailySize(List<Map> failedMovieList) {
        if (failedMovieList != null && !failedMovieList.isEmpty()) {
            List<Map> failedMoiveList = this.movieDailyAPI.saveDailyMovieDataByAPI(failedMovieList);
    ...
    public void movieWeeklySize(List<Map> failedMovieList) throws ParseException {
        if (failedMovieList != null && !failedMovieList.isEmpty()) {
            List<Map> failedMoiveList = this.movieWeeklyAPI.saveWeeklyMovieDataByAPI(failedMovieList, weeks);
            movieDailySize(failedMoiveList);
        }
    }
    ...
    public void webtoonData() {
        ...
        for (Day day1 : days) {
            List<WebtoonDayList> webtoonDayLists = this.webtoonDayListService.findBywebtoonDay(day1);
            if (webtoonDayLists.isEmpty()) {
                List<Long> webtoon = this.webtoonService.getWebtoonAPI(day1.getUpdateDays());
                this.webtoonDayListService.SaveWebtoonDayList(day1.getId(), webtoon);
            }
        }
    }
